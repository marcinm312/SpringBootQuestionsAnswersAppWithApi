...
@ComponentScan(basePackages = "pl.marcinm312.springdatasecurityex.controller.api")
@MockBeans({@MockBean(AnswerManager.class)})
...
class QuestionApiControllerTest {
	...
	void getQuestions_withAnonymousUser_unauthorized() throws Exception {
		mockMvc.perform(get("/api/questions")).andExpect(status().isUnauthorized()).andExpect(unauthenticated());
	}

	@Test
	@WithMockUser(username = "user")
	void getQuestions_simpleCase_success() throws Exception {
		String response = mockMvc.perform(get("/api/questions").with(httpBasic("user", "password")))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(authenticated().withUsername("user").withRoles("USER")).andReturn().getResponse()
				.getContentAsString();
	...
	void getQuestion_withAnonymousUser_unauthorized() throws Exception {
		mockMvc.perform(get("/api/questions/1000")).andExpect(status().isUnauthorized()).andExpect(unauthenticated()).andExpect(unauthenticated());
	}
	...
	void getQuestion_simpleCase_success() throws Exception {
		...
		String response = mockMvc.perform(get("/api/questions/1000").with(httpBasic("user", "password")))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(authenticated().withUsername("user").withRoles("USER")).andReturn().getResponse()
				.getContentAsString();
	...
	void getQuestion_questionNotExists_notFound() throws Exception {
		given(questionRepository.findById(2000L)).willReturn(Optional.empty());
		String receivedErrorMessage = Objects.requireNonNull(mockMvc.perform(get("/api/questions/2000").with(httpBasic("user", "password")))
				.andExpect(status().isNotFound()).andExpect(authenticated().withUsername("user").withRoles("USER"))
	...
	void createQuestion_withAnonymousUser_unauthorized() throws Exception {
		mockMvc.perform(post("/api/questions").contentType(MediaType.APPLICATION_JSON)
				...
				.characterEncoding("utf-8")).andExpect(status().isUnauthorized()).andExpect(unauthenticated());
	}
	...
	void createQuestion_simpleCase_success() throws Exception {
		...
		String response = mockMvc
				.perform(post("/api/questions").with(httpBasic("user", "password")).contentType(MediaType.APPLICATION_JSON)
						.content(mapper.writeValueAsString(questionToRequestBody)).characterEncoding("utf-8"))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(authenticated().withUsername("user").withRoles("USER")).andReturn()
	...
	void createQuestion_tooShortTitle_badRequest() throws Exception {
		given(userManager.getUserByAuthentication(any(Authentication.class)))
				.willReturn(UserDataProvider.prepareExampleGoodUser());
		mockMvc.perform(post("/api/questions").with(httpBasic("user", "password")).contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsString(QuestionDataProvider.prepareQuestionWithTooShortTitleToRequest()))
				.characterEncoding("utf-8")).andExpect(status().isBadRequest()).andExpect(authenticated().withUsername("user").withRoles("USER"));
	}
	...
	void deleteQuestion_withAnonymousUser_unauthorized() throws Exception {
		mockMvc.perform(delete("/api/questions/1000"))
	...
	void deleteQuestion_userDeletesHisOwnQuestion_success() throws Exception {
		...
		String response = mockMvc.perform(delete("/api/questions/1000").with(httpBasic("user", "password")))
				.andExpect(status().isOk()).andExpect(authenticated().withUsername("user").withRoles("USER"))
	...
	void deleteQuestion_administratorDeletesAnotherUsersQuestion_success() throws Exception {
		...
		String response = mockMvc.perform(delete("/api/questions/1000").with(httpBasic("administrator", "password")))
				.andExpect(status().isOk()).andExpect(authenticated().withUsername("administrator").withRoles("ADMIN"))
				.andReturn().getResponse().getContentAsString();
		Assertions.assertEquals("true", response);
	}
	...
	void deleteQuestion_userDeletesAnotherUsersQuestion_forbidden() throws Exception {
		...
		String receivedErrorMessage = Objects
				.requireNonNull(mockMvc.perform(delete("/api/questions/1000").with(httpBasic("user2", "password")))
						.andExpect(status().isForbidden()).andExpect(authenticated().withUsername("user2").withRoles("USER"))
	...
	void deleteQuestion_questionNotExists_notFound() throws Exception {
		given(userManager.getUserByAuthentication(any(Authentication.class))).willReturn(UserDataProvider.prepareExampleGoodUser());
		String receivedErrorMessage = Objects
				.requireNonNull(mockMvc.perform(delete("/api/questions/2000").with(httpBasic("user", "password")))
						.andExpect(status().isNotFound()).andExpect(authenticated().withUsername("user").withRoles("USER"))
						.andReturn().getResolvedException())
				.getMessage();
	...
	void downloadPdf_simpleCase_success() throws Exception {
		mockMvc.perform(get("/api/questions/pdf-export").with(httpBasic("user", "password")))
	...
	void downloadExcel_simpleCase_success() throws Exception {
		mockMvc.perform(get("/api/questions/excel-export").with(httpBasic("user", "password")))
				.andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_OCTET_STREAM))
